// Regression test: JVM class signature exceeds 65535-byte UTF8 constant pool limit
// when a graph has many @ContributesTo contributions and at least one generic supertype.
//
// Metro adds a MetroContributionTo<Scope> supertype for every contribution. The JVM only
// emits a generics signature when at least one supertype is parameterized, but once
// present, ALL supertypes are listed in a single signature string. Large graphs (1000+
// contributions) can exceed the 65535-byte UTF8 constant pool entry limit.
//
// This test reproduces the issue with 1000 @ContributesTo interfaces and one generic
// supertype. It passes with 500 contributions but fails with 1000.
